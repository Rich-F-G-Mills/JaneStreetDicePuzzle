...
* When determining the optimal strategy, the **expected future gain** for each of take and roll is considered; taking the value is considered the preferable strategy where this expected pay-off, considering all remaining turns, is at least as great as that for an otherwise roll.
...
* Any time the player chooses to bank the current value, the next turn (assuming there are turns remaining), must be a roll of the die.
...
* The recommended approach for any given combination of turn/roll depends on the prior action.
...
fnPlotDistributionClosingScores(variant2Analysis$CUMULATIVE_PROBS) +
  geom_vline(xintercept = variant2ClosingSummary$EXPECTED, color = 'red')
```
